...
$_SESSION['name']=$email;
    ...
    if($password != $confirm_password) {
        ...
        $select_users = mysqli_query($conn, "SELECT * FROM `register` WHERE email='". $_SESSION['name']. "'") or die('query failed');
        if(mysqli_num_rows($select_users) > 0) {
            $message[] = 'User already exists!';
        } else {
            mysqli_query($conn, "INSERT INTO `register`(name, email, password, user) VALUES('$name', '$email','$password', '$user')") or die('query failed');
